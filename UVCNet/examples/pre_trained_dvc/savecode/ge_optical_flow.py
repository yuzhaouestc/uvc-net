...
from dataset import EWAP_ethDataSet
    ...
    with torch.no_grad():
        ...
        for batch_idx, input in enumerate(test_loader):
            ...
            for i in range(seqlen):
                input_image = input_images[:, i, :, :, :]
                inputframe, refframe = Var(input_image), Var(ref_image)
                clipped_recon_image, mse_loss, warploss, interloss, bpp_feature, bpp_z, bpp_mv, bpp, estmv = net(inputframe, refframe)

                path = '/ai/base/data/wangfuchun/PyTorchVideoCompression/DVC/examples/pre_trained_dvc/test_compressed/' + str(batch_idx) + '_' + str(i)
                if not os.path.exists(path):
                    os.makedirs(path)
                torchvision.utils.save_image(inputframe, path + '/inputframe.png')
                torchvision.utils.save_image(clipped_recon_image, path + '/compressed.png')
                from flowlib import save_flow_image
                estmv = estmv[0].cpu().detach().numpy()
                # print('estmv.shape : ', estmv.shape)
                save_flow_image(estmv, path + '/estmv.png')
        ...
        sumpsnr /= cnt
        summsssim /= cnt
        log = "ETH dataset : average bpp : %.6lf, average psnr : %.6lf, average msssim: %.6lf\n" % (sumbpp, sumpsnr, summsssim)
        print(log)
        logger.info(log)
    return sumbpp, sumpsnr
...
if __name__ == "__main__":
    ...
    trains_lambada = [2048, 1024, 512, 256]
    crf_list = [19, 23, 27, 31]
    for i in range(4):
        if str(trains_lambada[i]) in args.pretrain:
            train_lambda = trains_lambada[i]
            crf = crf_list[i]
            # break
    assert crf is not None, "crf is None"
    print("crf : ", crf)
    gop = 10
    gop_list = [5, 10, 15, 20, 25, 30, 50, 100, 150]
    for gop in gop_list:
        test_dataset = EWAP_ethDataSet(crf=crf, gop=gop)
        bpp, psnr = test_eth()
        log_file = open(f'{train_lambda}_result.txt', 'a')
        result_str = 'trained_lambda: {}, crf : {},  gop : {}, bpp : {}, psnr : {}\n'.format(train_lambda, crf, gop, bpp, psnr)
        print(result_str)
