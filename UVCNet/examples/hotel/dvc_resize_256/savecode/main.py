...
import math
import json
from dataset import DataSet,  EWAP_ethDataSet, EWAP_hotel_DataSet
...
def testeth():
    ...
    for batch_idx, input in enumerate(test_loader):
        ...
        cnt += 1
        print(f'batch_idx : {batch_idx}, ref_bpp : {ref_bpp}, ref_psnr : {ref_psnr}, ref_msssim : {ref_msssim}')
        ...
        for i in range(seqlen):
            ...
            print(f'batch {batch_idx} frame {i} bpp {torch.mean(bpp).cpu().detach().numpy()} psnr {torch.mean(10 * (torch.log(1. / mse_loss) / np.log(10))).cpu().detach().numpy()}')
...
def train(epoch, global_step):
    ...
    #     'cheng2020-anchor': 
    # }
    I_net = cheng2020_anchor(quality=quality, pretrained=True).eval().to(device)
    for batch_idx, input in enumerate(train_loader):
        ...
        # rv['x_hat'].clamp_(0, 1)
        # out = rv
        # ref_image_hat = out['x_hat']
        # ref_image = ref_image_hat
...
if __name__ == "__main__":
    ...
    load_model(model, f'snapshot/{train_lambda}.model')
    if args.pretrain != '':
    ...
    if args.testeth:
        ...
        print('testing EWAP_eth')
        testeth()
        exit(0)

    if args.testhotel:
        test_dataset = EWAP_hotel_DataSet()
        print('testing EWAP_hotel')
        testeth()
        exit(0)
