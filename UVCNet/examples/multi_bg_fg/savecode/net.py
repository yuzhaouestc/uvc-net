...
class VideoCompressor(nn.Module):
    def __init__(self):
        ...
        self.mask_net.load_state_dict(torch.load('pretrained_masknet_model.pth'))
    ...
    def forward(self, fg_input_image, fg_referframe, quant_noise_feature=None, quant_noise_z=None, quant_noise_mv=None,  future_bg_ref_image=None, ns_train_bg_dis=None, next_I_frame=None,):
        self.mask_net.eval()
        mask = self.mask_net(fg_referframe)
        mu = mask
        threshold = 0.5
        mu = torch.where(mu>threshold, 1., 0. )
        mask = mu
        fullsize_referframe = fg_referframe
        fullsize_input_image = fg_input_image

        # # 使用上一帧的背景作为参考帧
        bg_ref_image = (1- mask) * fullsize_referframe
        # print('future_bg_ref_image.shape', future_bg_ref_image.shape)
        # print('ns_train_bg_dis.shape', ns_train_bg_dis.shape)
        if future_bg_ref_image is not None:
            bg_future_image = (1-mask) * future_bg_ref_image
            gop_size = 10
            bg_ref_weight =  1 - ns_train_bg_dis / gop_size * 0.5
            # bg_ref_weight =  ns_train_bg_dis / gop_size
            # bg_ref_weight [bg_ref_weight < 0.5 ] = 0.5
            # print('bg_ref_weight', bg_ref_weight)
            # print('ns_train_bg_dis', ns_train_bg_dis)
            # exit()
            # print('bg_ref_weight.shape', bg_ref_weight.shape)
            # print('bg_future_image.shape', bg_future_image.shape)
            # print('bg_ref_image.shape', bg_ref_image.shape)
            for i in range(bg_future_image.size(0)):
                bg_ref_image[i] = bg_ref_image[i] * bg_ref_weight[i]
                bg_future_image[i] = bg_future_image[i] * (1 - bg_ref_weight[i])

            bg_input_image = bg_ref_image + bg_future_image
        else:
            bg_input_image = bg_ref_image



        fg_input_image = mask * fg_input_image
        fg_referframe = mask * fg_referframe

        estmv = self.opticFlow(fg_input_image, fg_referframe)
        ...
        prediction, warpframe = self.motioncompensation(fg_referframe, quant_mv_upsample)

        input_residual = fg_input_image - prediction
        ...
        recon_fg_image = prediction + recon_res

        fullsize_recon_image = recon_fg_image + bg_input_image
        # recon_image = self.rec_net(recon_image)

        clipped_recon_image = fullsize_recon_image.clamp(0., 1.)
# distortion
        ...
        # mse_loss = torch.mean((fullsize_recon_image - fullsize_input_image).pow(2))
        mse_loss = torch.mean((clipped_recon_image - fullsize_input_image).pow(2))

        # psnr = tf.cond(
        #     tf.equal(mse_loss, 0), lambda: tf.constant(100, dtype=tf.float32),
        #     lambda: 10 * (tf.log(1 * 1 / mse_loss) / np.log(10)))

        warploss = torch.mean((warpframe - fg_input_image).pow(2))
        interloss = torch.mean((prediction - fg_input_image).pow(2))
