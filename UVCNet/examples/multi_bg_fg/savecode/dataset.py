...
class DataSet(data.Dataset):
    def __init__(self, path="data/vimeo_septuplet/test.txt", im_height=256, im_width=256):
        self.image_input_list, self.image_ref_list, self.fns_train_bg_ref, self.fns_train_bg_dis = self.get_vimeo(filefolderlist=path)
    ...
    def get_vimeo(self, rootdir="data/vimeo_septuplet/sequences/", filefolderlist="data/vimeo_septuplet/test.txt"):
        ...
        dataset = ['ewap_eth', 'ewap_hotel', 'train_HK']
        dataset = ['ewap_eth']
        for d in dataset:
            ...
            for folder in folder_list:
                ...
                start_num = 1
                gop_size = 10
                for i in range(start_num, len(imlist) - gop_size):
                    im_name = imlist[i]
                    im_id = int(im_name[:-4])
                    ...
                    bg_future_dis = random.randint(1, gop_size)
                    fns_train_bg_ref_number = refnumber + bg_future_dis
                    fns_train_bg_refname = str(fns_train_bg_ref_number) + '.png'
                    fns_train_bg_ref += [os.path.join(folder_path, fns_train_bg_refname)]
                    fns_train_bg_dis += [bg_future_dis]

        return fns_train_input, fns_train_ref, fns_train_bg_ref, fns_train_bg_dis
    ...
    def __getitem__(self, index):
        ...
        bg_ref_image = imageio.imread(self.fns_train_bg_ref[index])

        input_image = input_image.astype(np.float32) / 255.0
        ref_image = ref_image.astype(np.float32) / 255.0
        bg_ref_image = bg_ref_image.astype(np.float32) / 255.0

        input_image = input_image.transpose(2, 0, 1)
        ref_image = ref_image.transpose(2, 0, 1)
        bg_ref_image = bg_ref_image.transpose(2, 0, 1)


        
        input_image = torch.from_numpy(input_image).float()
        ref_image = torch.from_numpy(ref_image).float()
        bg_ref_image = torch.from_numpy(bg_ref_image).float()

        # input_image, ref_image = random_crop_and_pad_image_and_labels(input_image, ref_image, [self.im_height, self.im_width])
        input_image, ref_image, bg_ref_image = random_crop_and_pad_image_and_labels(input_image, ref_image, bg_ref_image, [self.im_height, self.im_width])
        ...
        # print('llll', bg_ref_image.shape)
        # exit()
        return input_image, ref_image, quant_noise_feature, quant_noise_z, quant_noise_mv, bg_ref_image, self.fns_train_bg_dis[index]
...
class EWAP_ethDataSet(data.Dataset):
    def __init__(self, root="data/surveillance/ewap_eth", filelist="data/UVG/originalv.txt", refdir='L12000', testfull=False, folder=['4']):
    ...
    def __getitem__(self, index):
        ...
        next_ref_image = None
        if index+1<len(self.ref):
            next_ref_image = imageio.imread(self.ref[index+1]).transpose(2, 0, 1).astype(np.float32) / 255.0
            next_ref_image = np.array(next_ref_image[:, :h, :w])
            return input_images, ref_image, self.refbpp[index], refpsnr, refmsssim, next_ref_image
