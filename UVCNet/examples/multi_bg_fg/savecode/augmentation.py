...
def random_crop_and_pad_image_and_labels(image, labels,  label2, size):
    combined = torch.cat([image, labels, label2], 0)
    ...
    freesize1 = random.randint(0,  max(size[1], image_shape[2]) - size[1])
    combined_crop = combined_pad[:, freesize0:freesize0 + size[0], freesize1:freesize1 + size[1]]
    # print('combined_crop shape: ', combined_crop.shape)
    # exit()
    return (combined_crop[:3, :, :], combined_crop[3:6, :, :],  combined_crop[6:, :, :])
